...
\begin{verbatim}
integral = univariate_integral_rk45(f, y0, theta)
\end{verbatim}
using 4th-order Runge-Kutta integrator(for nonstiff problems), or by

\begin{verbatim}
integral = univariate_integral_bdf(f, y0, theta)
\end{verbatim}

using BDF integrator(for stiff problems). Here $f$ is a
vector-value function, $y0$ is a vector of corresponding
size for integral constant, and $\theta$ contains the
integral limit, so that the output of
\texttt{univariate\_integral\_} is 

\begin{equation*}
  y_0 + \int_{\theta[0]}^{\theta[1]}{f}.
\end{equation*}
...
`\bf{transformed data}' {
  ...
  univar_integral = univariate_integral_rk45(func, y0, theta);
}
                                    $\vdots$			       
\end{lstlisting}
\end{fmpage}
