...
modelName <- "TwoCptModelPopulationTV"
...
weightMean = rnorm.trunc(nId, 70, 15, 50, 100)
...
## Generate time-dependent weight 
allData <- allData %>%
  left_join(data.frame(id = 1:nId, weightMean)) %>%
  mutate(weight = rnorm(nt, weightMean, 2))
