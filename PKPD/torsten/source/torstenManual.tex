...
where $y_1$, $y_2$, and $f$ are vector-valued functions, and  $y_1^\prime$ is independent of $y_2$. This structure arises, for instance, in PKPD models, where $y_1$ describes to the PK components and $y_2$ the PD effects. If $y_1$ has an analytical solution, we can construct a \textit{mix solver}, which analytically solves $y_1$ and numerically integrates $y_2$. This approach leads to an appreciable gain in computational efficiency. In the example of a Friberg-Karlsson semi-mechanistic model \cite{FK}, we observe an average $\sim 44\%$ speedup when using the mix solver in lieu of the numerical integrator\footnote{A manuscript on the subject is in progress.}.
...
where \texttt{ODE\_reduced\_system} is the system which gets numerically solved ($y_2$ in the above discussion) and \texttt{nOde} the number of equations in that system. Section 19.2 of the Stan user manual explains how to code an ODE system is the \texttt{functions} block of a Stan file. The function that defines a \textit{reduced system} has an almost identical signature but takes one additional argument: $y_1$, the solution to base PK ODEs (figure~\ref{reducedODESystem}).
...
`\bf{functions}'{
  real[] reducedODE(real t,  // time
			  ...
			  `\textcolor{red}{real[] y1,  // solution to base PK state}'
...
We cannot apply the mix solver to the Two Compartment example we have been using so far, but we'll discuss a more sophisticated case, where the mix solver comes in very handy, in the next section.
