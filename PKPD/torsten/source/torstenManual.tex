...
\texttt{<model\_name>(ODE\_reduced\_system, nOde, \\
\phantom{<model\_name>} time, amt, rate, ii, evid, cmt, addl, ss, \\
\phantom{<model\_name>} theta, biovar, tlag, \\                              
\phantom{<model\_name>} rel\_tol, abs\_tol, max\_step)}

where \texttt{ODE\_reduced\_system} is the system which gets numerically solved ($y_2$ in the above discussion) and \texttt{nOde} the number of equations in that system. Section 19.2 of the Stan user manual explains how to code an ODE system in the \texttt{functions} block of a Stan file. The function that defines a \textit{reduced system} has an almost identical signature but takes one additional argument: $y_1$, the solution to the base PK ODEs (figure~\ref{reducedODESystem}).
