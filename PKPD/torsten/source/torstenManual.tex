...
Torsten Version 0.83 beta \\ for Stan Version 2.15.0 \\ \ \\ 
\large May 2017 }
\end{center}
...
\begin{itemize}
  \item Stan version 2.15.0 (\url{http://mc-stan.org/})
  ...
  \item Mix compartment model, with analytical solutions to the base PK ODE sub-system and numerical solutions to the other ODEs, using built-in ODE integrators, with adjustable tuning parameters
\end{itemize}
...
\begin{itemize}
  \item Torsten is now up to date with Stan version 2.15.0
  \item Add ``mix solver" functions
  \item Add algebraic solver
  \item Torsten automatically corrects ODE functors when the rates are non-zero. The code carefully distinguishes the case where the rates are fixed data or random parameters. 
...
\begin{eqnarray}
  ...
  \frac{dy_2}{dt} &=& k_a y_1 - \left(\frac{CL}{V_2} + \frac{Q}{V_2}\right) y_2 +  \frac{Q}{V_3} y_3 \\
  \frac{dy_3}{dt} &=& \frac{Q}{V_2} y_2 - \frac{Q}{V_3} y_3
\end{eqnarray}

and the drug concentration is given by $c = \frac{y_2}{V_2}$.
...
\subsection{Mix ODE Model Function} \ \\
...
where $y_1$, $y_2$, $f_1$, and $f_2$ are vector-valued functions, and  $y_1^\prime$ is independent of $y_2$. This structure arises, for instance, in PKPD models, where $y_1$ describes the PK of the drug and $y_2$ the PD effects. If $y_1$ has an analytical solution, we can construct a \textit{mix solver}, which analytically solves $y_1$ and numerically integrates $y_2$. This approach leads to an appreciable gain in computational efficiency. In the example of a Friberg-Karlsson semi-mechanistic model \cite{2364}, we observe an average $\sim 44\%$ speedup when using the mix solver in lieu of the numerical integrator\footnote{A manuscript on the subject is in progress.}.

Torsten supports the mix solver for cases where $y_1$ is either a One or Two Compartment model with a first-order absorption.
...
where \texttt{reduced\_ODE\_system} specifies the system we  numerically solve ($y_2$ in the above discussion) and \texttt{nOde} the number of equations in that system. Section 19.2 of the Stan user manual explains how to code an ODE system in the \textbf{functions} block. The function that defines a \textit{reduced system} has an almost identical signature but takes one additional argument: $y_1$, the solution to the base PK ODEs (figure~\ref{reducedODESystem}).
...
`\bf{functions}'{
  real[] reducedODE(real t,  // time
			  ...
			  `\textcolor{red}{real[] y1,  // solution to the base PK states}'
...
These four functions correspond to all the permutations we can obtain when using either a base One or Two Compartment function, and the Runge-Kutta 4th/5th order (rk45) or Backward Differentiation (BDF) integration method.

We cannot apply the mix solver to the Two Compartment example we have been using so far. Instead, we'll consider the very model which motivated the implementation of the method in the first place.
...
\begin{eqnarray}
   \frac{dy_{\mathrm{prol}}}{dt} &=& k_{\mathrm{prol}} y_{\mathrm{prol}} (1 - E_{drug})\left(\frac{Circ_0}{y_{\mathrm{circ}}}\right)^\gamma - k_{\mathrm{tr}}y_{\mathrm{prol}} \\
...
The ODEs specifying the Two Compartment Model (equations 1 - 3) do not depend on the PD ODEs (equations 4 - 8) and can be solved analytically by Torsten. We therefore specify our model using a mix solver function. We do not expect our system to be stiff and use the Runge-Kutta 4th/5th order integrator.
...
transformed parameters {
  ...
  x = `\textcolor{red}{mixOde2CptModel\_rk45}'(FK_ODE, nOde,
                                   time, amt, rate, ii, evid, cmt, addl, ss,
                                   theta, biovar, tlag,
                                   1e-6, 1e-6, 1e+6);
...
We now return to example 2 and extend it to a population model. While we recommend using the mix solver, for completeness we'll show how to specify the model with the \texttt{generalOdeModel\_*} function. We leave it as a (trivial) exercise to the reader to rewrite the model with \texttt{mixOde2CptModel\_*}. 
