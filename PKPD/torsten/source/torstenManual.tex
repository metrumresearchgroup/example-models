...
\definecolor{MRGGreen}{rgb}{0, 0.350, 0.125}
\parskip 10pt
\parindent 0em
...
User Manual \\ \ \\ \ \\ \ \\
\Large Torsten Version 0.83 beta \\ for Stan Version 2.16.0 \\ \ \\
\large July 2017 }
\end{center}
...
\subsubsection{Intalling Torsten with RStan} The easiest way to install the RStan interface with Stan and Torsten is to run the script \texttt{R/setupRTorsten.R}. You'll need to make a few minor adjustments, notably by specifying the location at which you wish to install RStan (and its dependency StanHeaders). You may run into difficulty if Torsten is not up to date with Stan's last release\footnote{See issue \#3 on our GitHub issue tracker: \url{https://github.com/charlesm93/example-models/issues/3}}.
...
\subsection{Overview} \ \\ \ \\
Torsten is a prototype Pharmacokinetic/Pharmacodynamic (PK/PD) model library for use in Stan 2.16.0. The current version includes:
\begin{itemize}
  ...
  \item Mix compartment model with forcing PK function described by a One or Two compartment model, and forced states described by a system of first-order ODEs.
 \end{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item For functions which use a numerical integrator (the general and mix compartment models) the following limitations apply when calculating a steady state solution:
    \begin{itemize}
      \item The infusion time must be shorter than the inter-dose interval.
      \item In the case of a multiple truncated infusion dosing regimen, the bioavailability (F) must be fixed,
      \item The rate vector must be fixed,
    \end{itemize}
  \end{itemize}
...
\begin{itemize}
  ...
  \item General linear compartment models with semi-analytical solutions using a built-in Matrix Exponential function
...
\begin{itemize}
  ...
  \item Steady state solution for general compartment model
...
The data are generated using the R package \textit{mrgsolve} \cite{mrgsolve2017}, see \texttt{TwoCptModelSimulation.R}. We show the results obtained when using the function \texttt{PKModelTwoCpt}, which computes solutions to the ODEs analytically.
...
where $K$ is a matrix. For example $K$m for a two compartment model (equation~\ref{eq:TwoCpt}) with first order absorption is: \\
...
Torsten can compute the steady state solution for a general compartment model, with a few notable exceptions, which will return an error message. To compute a steady state solution, the following conditions should be met:
\begin{itemize}
  \item the \texttt{rate} argument is passed as an array of fixed data
  \item the infusion time ($amt/rate$) is \underline{shorter} than the inter-dose interval ($ii$)
  \item in the case of a multiple trunctaed infusions dosing regimen, \texttt{biovar} is an array of fixed data.
\end{itemize}
...
where \texttt{reduced\_ODE\_system} specifies the system we  numerically solve ($y_2$ in the above discussion) and \texttt{nOde} the number of equations in that system. The function that defines a \textit{reduced system} has an almost identical signature to that used for a full system, but takes one additional argument: $y_1$, the PK states, i.e. solution to the PK ODEs (figure~\ref{reducedODESystem}).
