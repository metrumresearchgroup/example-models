...
modelDir <- scriptDir
dataDir <- scriptDir
outDir <- file.path(modelDir, torsten.ver)
figDir <- file.path(outDir, "figure")
tabDir <- file.path(outDir, "table")
toolsDir <- "../R/tools"

stanDir <- "/Users/yiz/Work/temp/temp/Torsten/cmdstan"
...
library(rstan)
mclapply(chains,
         function(chain, model, data, iter, warmup, thin, init, tempDir = "."){
